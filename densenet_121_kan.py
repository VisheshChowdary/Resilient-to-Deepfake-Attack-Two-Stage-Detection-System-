...
class DenseNet121KAN(nn.Module):
    def __init__(self, num_classes=2):
        super(DenseNet121KAN, self).__init__()
        
        # Load pre-trained DenseNet121
        densenet = models.densenet121(weights=models.DenseNet121_Weights.IMAGENET1K_V1)
        
        # Extract all features (CNN layers)
        self.features = densenet.features
        
        # Freeze layers up to Dense Block 3
        for i, layer in enumerate(self.features):
            if i < 10:  # This covers up to Dense Block 3
                for param in layer.parameters():
                    param.requires_grad = False
            else:  # Dense Block 4 and beyond
                for param in layer.parameters():
                    param.requires_grad = True
        
        print("\nLayer Freezing Summary:")
        print("Frozen: Initial layers up to Dense Block 3")
        print("Trainable: Dense Block 4 and KAN layers")
        
        # The output of DenseNet121 features is 1024 channels
        ...
        # Add KAN layers
        self.kan = KAN(
            layers_hidden=[512, 256, num_classes],  # Input size matches FC output
            grid_size=5,
            spline_order=3,
            scale_noise=0.1,
            scale_base=1.0,
...
def create_model(num_classes=2):
    model = DenseNet121KAN(num_classes=num_classes)
    return model
...
def create_data_loaders(batch_size=32):
    ...
    train_dataset = DeepfakeDataset('dataset/data/train', transform=transform)
    val_dataset = DeepfakeDataset('dataset/data/val', transform=transform)
...
def train_model(model, train_loader, val_loader, num_epochs=25, patience=5, learning_rate=0.001):
